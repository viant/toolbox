...
// PackInt32sTo64 returns args<<32|args
func PackInt32sTo64(source interface{}, state data.Map) (interface{}, error) {
	...
	case string:
		...
		if len(args) != 2 {
			return nil, fmt.Errorf("expected 2 int values, but had: %v", len(args))
		}
		ints := make([]int, 2)
